...
class MusicViewModel(private val musicRepository: MusicRepository) : ViewModel() {
    ...
    private val _artistListLiveData: MutableLiveData<Resource<List<ArtistResult>>> = MutableLiveData()
    ...
    fun findTracksByName() {
        ...
        viewModelScope.launch {
            delay(1000)
            _trackListLiveData.postValue(Resource.success(musicRepository.getTracksByName(searchName.value)))
        }
    }
    ...
    fun findArtistsByName() {
        ...
        viewModelScope.launch {
            delay(1000)
            _artistListLiveData.postValue(Resource.success(musicRepository.getArtistsByName(searchName.value)))
        }
    }
